...
public class ENCR_reordering extends RichFlatMapFunction<
    ...
    public void flatMap(Tuple7<Long, String, String, Long, Boolean, CallDetailRecord, Double> tuple, Collector<Tuple7<Integer, Long, String, Long, Double, CallDetailRecord, Double>> collector) {
        ...
        if (cdr.callEstablished) {
            ...
            if (tuple.f6 >= 0) {
                Tuple7<Integer, Long, String, Long, Double, CallDetailRecord, Double> out = new Tuple7<>(ScorerMap.ECR, timestamp, caller, cdr.answerTimestamp, tuple.f6, cdr, -1.0);
            ...
            if (newCallee) {
                ...
                Tuple7<Integer, Long, String, Long, Double, CallDetailRecord, Double> out = new Tuple7<>(ScorerMap.ENCR, timestamp, caller, cdr.answerTimestamp, rate, cdr, -1.0);
                collector.collect(out);
                LOG.debug("tuple out: {}", out);
            }
        }
