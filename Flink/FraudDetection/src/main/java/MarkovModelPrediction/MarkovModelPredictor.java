...
public class MarkovModelPredictor extends ModelBasedPredictor {
    ...
    public MarkovModelPredictor(Configuration conf) {
        ...
        localPredictor = conf.getBoolean(LOCAL_PREDICTOR, true);

        if (localPredictor) {
            stateSeqWindowSize = conf.getInteger(STATE_SEQ_WIN_SIZE, 5);
        }  else {
        ...
        stateOrdinal = conf.getInteger(STATE_ORDINAL, 1);

        //detection algoritm
        String algorithm = conf.getString(DETECTION_ALGO, "missProbability");
