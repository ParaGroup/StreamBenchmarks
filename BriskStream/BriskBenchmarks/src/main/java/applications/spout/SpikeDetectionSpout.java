...
public class SpikeDetectionSpout extends AbstractSpout {
    private static final Logger LOG = LoggerFactory.getLogger(SpikeDetectionSpout.class);
    ...
    public SpikeDetectionSpout() {
        super(LOG);
        this.scalable = false;
        this.parser = new SensorParser();
        this.fields = new Fields(Field.DEVICE_ID, Field.VALUE, Field.TIMESTAMP);
    }

    public SpikeDetectionSpout(Parser _parser, Fields _fields) {
        super(LOG);
        this.scalable = false;
        this.parser = (SensorParser) _parser;
        this.fields = _fields;
    }
    ...
    public void initialize(int thread_Id, int thisTaskId, ExecutionGraph graph) {
        ...
        String s = System.getProperty("user.home").concat("/StreamBenchmarks/Datasets/SD/sensors.dat");
        array = new ArrayList<>();
        try {
            openFile(s);
...
//                        (StateWrapper<List<StreamValues>>) ClassLoaderUtils.newInstance(parserClass, "wrapper", LOG, tuple_size);
    ...
    public void nextTuple() throws InterruptedException {
        ...
        if (emit != null) {
            ...
            //emit2[0] = 0;
            collector.emit(0, emit2); // fields: DEVICE_ID(int), VALUE(double), TIMESTAMP(long)
            sent++;
