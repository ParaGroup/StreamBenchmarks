...
public class TrafficMonitoringSpout extends AbstractSpout {
    private static final Logger LOG = LoggerFactory.getLogger(TrafficMonitoringSpout.class);
    ...
    public TrafficMonitoringSpout() {
        super(LOG);
        this.scalable = false;
        this.parser = new TaxiTraceParser();
        this.fields = new Fields(Field.VEHICLE_ID, Field.DATE_TIME, Field.OCCUPIED, Field.SPEED, Field.BEARING, Field.LATITUDE, Field.LONGITUDE, Field.TIMESTAMP);
    }

    public TrafficMonitoringSpout(Parser _parser, Fields _fields) {
        super(LOG);
        this.scalable = false;
        this.parser = (TaxiTraceParser) _parser;
        this.fields = _fields;
    }
    ...
    public void initialize(int thread_Id, int thisTaskId, ExecutionGraph graph) {
        ...
        String s = System.getProperty("user.home").concat("/StreamBenchmarks/Datasets/TM/taxi-traces.csv");
        array = new ArrayList<>();
        try {
            openFile(s);
...
//                        (StateWrapper<List<StreamValues>>) ClassLoaderUtils.newInstance(parserClass, "wrapper", LOG, tuple_size);
    ...
    public void nextTuple() throws InterruptedException {
        ...
        collector.emit(0, emit2); // 8 fields
        sent++;
        counter++;
