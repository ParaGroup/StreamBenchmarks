...
public class MemFileSpout extends AbstractSpout {
    ...
    public void initialize(int thread_Id, int thisTaskId, ExecutionGraph graph) {
        ...
        String s = System.getProperty("user.home").concat("/StreamBenchmarks/Datasets/").concat(path);
...
//		if (control > 0) {
        collector.emit(array_array[counter], System.nanoTime());//Arrays.copyOf(array_array[timestamp_counter], array_array[timestamp_counter].length) a workaround to ensure char array instead of string is used in transmission.
...
//		collector.emit(array[timestamp_counter]);//Arrays.copyOf(array[timestamp_counter], array[timestamp_counter].length) a workaround to ensure char array instead of string is used in transmission.
        collector.emit_nowait(array_array[counter], System.nanoTime());
        counter++;
        if (counter == array_array.length) {
            counter = 0;
        }
