...
public class WordCountSpout extends AbstractSpout {
    ...
    public WordCountSpout() {
        super(LOG);
        this.scalable = false;
        this.parser = new StringParser();
        this.fields = new Fields(Field.SENTENCE, Field.TIMESTAMP);
    }

    public WordCountSpout(Parser _parser, Fields _fields) {
        super(LOG);
        this.scalable = false;
        this.parser = (StringParser) _parser;
        this.fields = _fields;
    }
    ...
    public void initialize(int thread_Id, int thisTaskId, ExecutionGraph graph) {
        ...
        String s = System.getProperty("user.home").concat("/StreamBenchmarks/Datasets/WC/book.dat");
        array = new ArrayList<>();
        try {
            openFile(s);
        }
...
//                        (StateWrapper<List<StreamValues>>) ClassLoaderUtils.newInstance(parserClass, "wrapper", LOG, tuple_size);
    ...
    public void nextTuple() throws InterruptedException {
        ...
        char[] emit = parser.parse(line);
        Object[] emit2 = new Object[2];
        emit2[0] = emit;
        emit2[1] = System.nanoTime();
        collector.emit(0, emit2); // fields: SENTENCE(char[]), TIMESTAMP(long)
        counter++;
        sent++;
        bytes += new String(line).getBytes().length;
